...
import com.insightsurfface.videocrawler.adapter.OnlyTextListAdapter;
        ...
        if (null == onlyTextListAdapter) {
            ...
            optionsLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    if (null != mOnListDialogListener) {
                        if (null != list && list.length > 0) {
                            mOnListDialogListener.onItemClick(position);
                            mOnListDialogListener.onItemClick(list[position]);
                            if (null != codeList && codeList.length > 0) {
                                mOnListDialogListener.onItemClick(list[position], codeList[position]);
                            }
                            dismiss();
                        }
                    }
